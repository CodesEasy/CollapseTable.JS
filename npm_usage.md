...
This guide covers installing and using `@codeseasy/collapsetable` with **npm**, plus framework patterns for **React**, **Next.js**, **Vue 3**, **Svelte**, and **Angular**. For CDN usage and general docs, see `README.md`.
...
### CommonJS (Webpack, Node older configs)
...
See `README.md` for full option defaults and payloads.

---

## 6) TypeScript tips

Types are shipped in `src/types/collapsetable.d.ts`. Useful imports:
...
const ct = new CollapseTable<Options>(); // generic not required; included for clarity
```

Strongly-typed hooks:

```ts
ct.on('refit', (e: CollapseTable.RefitEventPayload) => {
...
Use a ref to the table and `useEffect` for lifecycle. Clean up with `unset`.
...
**Custom details rendering in React:** `detailsRender` expects a **DOM Node** or HTML string. If you must render React inside, render into a detached element:

```tsx
detailsRender(row, hiddenCols, cells) {
  const root = document.createElement('div');
  root.className = 'react-details';
  // Optional: hydrate with your own renderer
  // createRoot(root).render(<MyDetails row={row} hiddenCols={hiddenCols} cells={cells} />);
  // If not using React here, return simple HTML:
...
> Don’t rely on React’s synthetic events inside `detailsRender`; attach plain DOM listeners if needed.

---

### 7.2 Next.js / Remix (SSR)

Because the library manipulates the DOM, run it on the **client only**.
...
**Alternatively**, dynamically import on the client:
...
**Remix:** put the initialization in `useEffect` similarly; avoid running in the loader.
...
export class OrdersComponent implements AfterViewInit, OnDestroy {
  ...
  // Call this after data updates that affect columns/widths:
  refresh(): void {
...
## 8) Dynamic data & lifecycle best-practices
...
* If the table sits in a **hidden container** (`display:none` tabs/modals), `deferWhenHidden: true` (default) defers precise fitting. Call `refresh()` when the container becomes visible if needed.
* If you **add/remove columns** or change header attributes (`data-priority`, `data-min`), just call `ct.refresh(table)`; the library re-measures and refits. If you structurally alter `<thead>`, `MutationObserver` will auto-rebuilt metadata; `refresh()` is still a good idea after your update.
...
> The function receives the **row `<tr>`**, an array of **hidden column metadata**, and the current **cells**. Return **`Node`** or **`string`**. Return nothing to keep the default renderer.

---

## 11) Styling & theming (npm)
...
Example (override button glyphs/spacing):

```css
.ctbl .ctbl-toggle { padding: 6px 10px; font-weight: 600; }
```
...
* **Tree-shaking:** The build is small; UMD is fine for modern bundlers. No side-effectful CSS files; the library injects its own base CSS once.
* **SSR:** Safe to import into SSR builds, but **only call** `set(...)` on the **client**. In frameworks, use `useEffect`/`onMounted`/`onMount`/`ngAfterViewInit`.
* **Transpilation:** No special config required. If you transpile `node_modules`, exclude this package—it already targets modern browsers.

---

## 13) Troubleshooting (npm context)
...
* **`colspan`/`rowspan`:** Collapsing is disabled when spans are detected in header/body (console warning). Base styles still apply.

---

## 14) Advanced: programmatic control
